...
// ================================
// PrelU Versions of Timing Functions
// ================================

#ifdef __x86_64__
float rdtsc_prelu(comp_func_prelu func_to_test, float *X_buf, float *B_buf, float *alpha_buf, float *Y_buf, int M_arg, int N_arg, int K_arg)
{
    int i, num_runs_actual;
    myInt64 cycles_val;
    myInt64 start_val;
    ...
    while (num_runs_actual < (1 << 14))
    {
        ...
        for (i = 0; i < num_runs_actual; ++i)
        {
            func_to_test(X_buf, B_buf, alpha_buf, Y_buf, M_arg, N_arg, K_arg);
        }
        cycles_val = stop_tsc(start_val);
...
#ifdef __aarch64__
float rdvct_prelu(comp_func_prelu func_to_test, float *X_buf, float *B_buf, float *alpha_buf, float *Y_buf, int M_arg, int N_arg, int K_arg)
{
    int i, num_runs_actual;
    TIMESTAMP cycles_val;
    TIMESTAMP start_val;
...
#ifdef PMU
struct performance_counters rdpmu_prelu(comp_func_prelu func_to_test, float *X_buf, float *B_buf, float *alpha_buf, float *Y_buf, int M_arg, int N_arg, int K_arg)
{
    kperf_init();
    int i, num_runs_actual;
    struct performance_counters startperf, endperf, result;
...
float c_clock_prelu(comp_func_prelu func_to_test, float *X_buf, float *B_buf, float *alpha_buf, float *Y_buf, int M_arg, int N_arg, int K_arg)
{
    int i, num_runs_actual;
    float cycles_val;
    clock_t start_clk, end_clk;
...
#ifndef WIN32
float timeofday_prelu(comp_func_prelu func_to_test, float *X_buf, float *B_buf, float *alpha_buf, float *Y_buf, int M_arg, int N_arg, int K_arg)
{
    int i, num_runs_actual;
    float cycles_val;
    struct timeval start_tv, end_tv;
...
float gettickcount_prelu(comp_func_prelu func_to_test, float *X_buf, float *B_buf, float *alpha_buf, float *Y_buf, int M_arg, int N_arg, int K_arg)
{
    int i, num_runs_actual;
    float cycles_val, start_tc, end_tc;
...
float queryperfcounter_prelu(comp_func_prelu func_to_test, float *X_buf, float *B_buf, float *alpha_buf, float *Y_buf, int M_arg, int N_arg, int K_arg, LARGE_INTEGER f)
{
    int i, num_runs_actual;
    float cycles_val;
    LARGE_INTEGER start_pc, end_pc;
...
float perf_test_prelu(comp_func_prelu f, int M_param, int K_param, int N_param, int nonZero)
{
    srand((unsigned)time(NULL));
    ...
    vector<float> Y_perf(M_param * N_param, 0);
    vector<float> B_perf(N_param, 2);
    vector<float> alpha_perf(N_param, 0.1); // Initialize alpha values for PReLU

    Y_perf.insert(Y_perf.end(), 10, 0);
    X_perf.insert(X_perf.end(), 10, 0);

#ifdef __x86_64__
    return rdtsc_prelu(f, X_perf.data(), B_perf.data(), alpha_perf.data(), Y_perf.data(), M_param, N_param, K_param);
#elif defined(__aarch64__) && defined(PMU)
    struct performance_counters p = rdpmu_prelu(f, X_perf.data(), B_perf.data(), alpha_perf.data(), Y_perf.data(), M_param, N_param, K_param);
    printf("\n"
           "Instructions     : %.3e\n"
           "Branches         : %.3e\n"
           "Branch Misses    : %.3e\n"
    ...
    return p.cycles;
#elif defined(__aarch64__)
    return rdvct_prelu(f, X_perf.data(), B_perf.data(), alpha_perf.data(), Y_perf.data(), M_param, N_param, K_param);
#elif defined(_WIN32) || defined(WIN32)
    LARGE_INTEGER freq;
    QueryPerformanceFrequency(&freq);
    return queryperfcounter_prelu(f, X_perf.data(), B_perf.data(), alpha_perf.data(), Y_perf.data(), M_param, N_param, K_param, freq) * (FREQUENCY / (float)freq.QuadPart);
#else // Generic Unix
    return timeofday_prelu(f, X_perf.data(), B_perf.data(), alpha_perf.data(), Y_perf.data(), M_param, N_param, K_param) * FREQUENCY;
#endif
}