...
void fill_cache_region_with_random_data(std::vector<uint8_t>& buffer) {
    ...
    for (size_t i = 0; i < buffer.size(); i += CACHE_LINE_SIZE_BYTES) {
        ...
        for (size_t j = 0; j < CACHE_LINE_SIZE_BYTES; ++j) {
            if (i + j < buffer.size()) { // Ensure we don't write out of bounds
...
void compulsory_only() {
    ...
    vector<int> W_raw = generateSparseMatrix<int>(K, N, NONZERO, true); // For SparseFormat
    ...
    vector<float> Y_main(M * N, 0); // Renamed refY
        ...
        int y = sf_csr_data->col_start_pos[rand() % sf_csr_data->col_start_pos.size()];
    #endif

    volatile int x = X_main.data()[rand() % X_main.size()] 
    + B_main.data()[rand() % B_main.size()] 
    + Y_main.data()[rand() % Y_main.size()]
    + y
    + W_raw.data()[rand() % W_raw.size()];
    exit(x % 255);
#endif
}
...
int main(int argc, char **argv)
{
    ...
    volatile int x = X_main.data()[rand() % X_main.size()] 
          + B_main.data()[rand() % B_main.size()] 
          + W_FP32_main.data()[rand() % W_FP32_main.size()] 
          + Y_main.data()[rand() % Y_main.size()]
          + sf_csr_data->col_start_pos[rand() % sf_csr_data->col_start_pos.size()]
          + W_raw.data()[rand() % W_raw.size()];
